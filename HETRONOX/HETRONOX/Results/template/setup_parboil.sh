...
do
              mkdir $benchmark
              cd $benchmark
              # ln -s /home/cy0194/benchmarks/parboil/benchmarks/$benchmark/build/cuda_default/$benchmark .
	          cp /home/cy0194/cpugpusim1/bash-scripts/run_scripts/parboil/mainscript_$benchmark .
...
if [ $1 = "--cleanup" ]; then
    ...
    for BMK in cutcp lbm histo mm sad spmv tpacf; do
...
# fft
for BMK in cutcp histo lbm mm sad spmv tpacf; do
    if [ -f $BMK/gpgpusim.config ]; then
