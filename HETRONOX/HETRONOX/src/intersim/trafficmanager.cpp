...
//#include "../cpu-sim/sharedglobals.h"

//Turns on flit tracking!
//#ifndef DEBUG
//Onur Nachi modified
#define DEBUG 0
//#endif
...
TrafficManager::TrafficManager( const Configuration &config, Network *net , int u_id, int seperate_network_ratio)
: Module( 0, "traffic_manager" )
{
   ...
   for ( s = 0; s < _sources; ++s ) {
      ...
      _buf_states[s] = new BufferState( config, this, tmp_name.str( ), seperate_network_ratio, uid );
   ...
   //UNT added
   _latency_stats_sep = new Stats * [36];
  ...
  for ( int j = 0; j < 36; ++j ) {
     _latency_stats_sep[j] = new Stats( this, tmp_name.str( ), 1.0, 1000 );
     tmp_name.seekp( 0, ios::beg );   
  }
   ...
   _routing_function  = GetRoutingFunction( config, seperate_network_ratio, uid  );
...
TrafficManager::~TrafficManager( )
{
   ...
   for ( int i = 0; i < 36; ++i ) {
      delete _latency_stats_sep[i];
   }

   delete [] _latency_stats_sep;
   ...
   if ( f->tail || _flit_timing ) {
      ...
      if ( ( _sim_state == warming_up ) || f->record ) {
         if ( f->true_tail || _flit_timing ) {
            ...
            case none:
               _latency_stats_sep[f->src]->AddSample( (_time - f->time) );
...
void TrafficManager::_GeneratePacket( int source, int psize /*# of flits*/ , 
                                      int cl, int time, void* data, int dest, int _nets )
{
   Flit *f;
   bool record;
   bool split_head;
   ...
   for ( int i = 0; i < psize; ++i ) {
      ...
      //Energy
      if (coRun) {
         p_photonic += (8*0.35 + 28*0.34 + 0.12 * 8 + 0.35 * 8);
      }

      //UNT added
      f->net_s = _nets; //Used for identifing the CPU or GPU flits
   ...
   for ( int input = 0; input < _net->NumSources( ); ++input ) {
      ...
      if ( !_partial_packets[input][highest_class].empty( ) ) {
         ...
         if ( f->head && ( f->vc == -1 ) ) { // Find first available VC
            f->vc = _buf_states[input]->FindAvailable( f );
...
{     //some initialization parts of _SingleSim for gpgpgusim
	...
	// Onur added: never reinit icnt
	// FIXXX: check correctness. What are the implications on _ClearStats?
	// Looks like the previous network stats were per-kernel. Now it is cumulative.
	if (_time > 0) {
...
void TrafficManager::ShowOveralStat( )
{
   ...
   for ( int i = 0; i < 36; ++i) {
      cout << "% Average latency of Each core:"<<_latency_stats_sep[i]->Average()<<endl;
   }  
